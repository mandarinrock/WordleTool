...
def guessChecker(guess, results):
    ...
    global answerList
    global numordle
    previousList = copy.copy(answerList[0])
                    ...
                    if letter in greens:
                        ...
                        for position in range(len(greens)):
                            ...
                            if word[position] == letter and greens[position] != letter:
                                ...
                                if word in answerList[0]:

                                    answerList[0].remove(word)
                                    bCount += 1
    ...
    if len(answerList[0]) == 1:
        print("The answer is: " +  answerList[0][0] + "\n")
        quit()
    elif len(answerList[0]) < 1:
        print("Something went wrong")
        quit()
    else:
        frequency(answerList[0])
...
def frequency(inputCombo):
    ...
    # print("\nThe best guess is " + '\033[1m' + '\x1B[3m' + best[2] + '\x1B[0m')
    print("\nThere are " + str(len(order)) + " answers remaining")
    print("Press " + '\033[1m' + "enter" + '\x1B[0m' + " to view all valid answers")
    print("Or enter your own custom guess")
    userInput = input("Or enter the results for " + '\033[1m' + '\x1B[3m' + best[2] + '\x1B[0m' + " as [B/G/Y]: ")
    if userInput == "":
        print("The remaining valid answers are:")
        for combo in order:
            print(combo[2])
        print("\nEnter your own custom guess")
        userInput = input("Or enter results for " + '\033[1m' + '\x1B[3m' + best[2] + '\x1B[0m' + " as [B/G/Y]: ")
    for letter in userInput:
        if letter != 'b' and letter != 'g' and letter != 'y':
            nextGuess = userInput
            userInput = input("Enter results for " + '\033[1m' + nextGuess + '\x1B[0m' + " as [B/G/Y]: ")
            break

    guessChecker(nextGuess, userInput)

def multiSolver(startingWord):

    global numordle

    for i in range(numordle):

        userInput = input("Enter results as [B/G/Y]: ")
        guessChecker(startingWord, userInput)
...
def main():
    ...
    firstWords = copy.copy(startingWords)

    print("How many answers are you trying to find?")
    userInput = input("For regular Wordle press " + '\033[1m' + "enter" + '\x1B[0m' + ": ")
    if userInput == "": numordle = 1
    else: numordle = userInput

    for i in range(numordle):
        answerList.append(copy.copy(officialList))

    print("\nPress " + '\033[1m' + "enter" + '\x1B[0m' + " for a random first word")
    print("Or enter a random seed")
    userInput = input("Or enter your own first word: ")

    if userInput == "":
        firstWord = random.choice(firstWords)
        print("Enter: " + '\033[1m' + '\x1B[3m' + firstWord + '\x1B[0m')
